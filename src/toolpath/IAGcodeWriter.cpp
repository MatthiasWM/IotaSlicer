...
void IAGcodeWriter::sendInitSequence(unsigned int toolmap)
{
    ...
    // count the bit in toolmap
    uint32_t v = toolmap;
    v = v - ((v >> 1) & 0x55555555);                    // reuse input as temporary
    v = (v & 0x33333333) + ((v >> 2) & 0x33333333);     // temp
    pToolCount = ((v + (v >> 4) & 0xF0F0F0F) * 0x1010101) >> 24; // count
#ifdef IA_QUAD
#error
...
void IAGcodeWriter::requestTool(int t)
{
    if (pT!=t) {
        ...
        if (t!=-1) {
            ...
            if (pToolCount && pPrinter->toolChangeStrategy()==3) { // prime tower
                /// \bug the prime tower must ALWAYS be built, or a few layers without a toolchange will disrupt the tower
                double tw = 13.0, td = 13.0;
                double dx = (tw + 1.0) * t;
                // snake lines
                for (double i=1; i<=tw-1; i++) {
                    cmdRapidMove(pause.x()+dx+i, pause.y()-0.5);
                    cmdPrintMove(pause.x()+dx+i, pause.y()-td+0.5);
                    cmdPrintMove(pause.x()+dx+i+0.5, pause.y()-td+0.5);
                    cmdPrintMove(pause.x()+dx+i+0.5, pause.y()-0.5);
                }
                // a rectangle around the prime tower
                cmdRapidMove(pause.x()+dx, pause.y());
                cmdPrintMove(pause.x()+dx+tw, pause.y());
                cmdPrintMove(pause.x()+dx+tw, pause.y()-td);
                cmdPrintMove(pause.x()+dx, pause.y()-td);
